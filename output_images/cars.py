...
def check_is_same_car(car1_center, car2_center):
...
class Cars:
    ...
    def match_cars_with_tracked(self, center):
        match = False
        car = None
        for known_car in self.cars_tracked:
            if check_is_same_car(center, known_car[1]):
                known_car[4] = 1
                print("Found match")
                car = known_car
                match = True
    ...
    def updateCarsTracked(self, car_list):
        ...
        if len(self.cars_tracked) == 0:
            ...
            for car in car_list:
                match, known_car = self.match_cars_with_tracked(car[1])
                if match:
                    car[4] = 1
                    car[3] = 0
                    car[2] = known_car[2] + 1
