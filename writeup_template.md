...
One thing that worked for me was to use probability classifier and set a threshold for match to > 90% probabilty.  This only worked because the data used to train the classifier was of high quality.  This along with the fact that I setup overlap of 90% between sliding windows and setting heatmap threshold of 3 eliminated pretty much all false positives.  Another problem was missed detections. To overcome this tracking cars and displaying them even when are not detected with some tolerance before its retired.  This can be further improved.  Also bounding boxes for cars can be improved as well.
